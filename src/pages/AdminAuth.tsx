...
const AdminAuth = ({ mode }: { mode: 'login' | 'register' | 'forgot' | 'reset' }) => {
  ...
  const subtitle = {
    login: 'Access the private Nexora operations dashboard.',
          ...
          {(mode === 'login' || mode === 'register' || mode === 'forgot') && (
            <label className="block">
              ...
              <div className="relative mt-2">
                ...
                <input type="email" name="email" required className="theme-input w-full rounded-xl pl-10 pr-4 py-3 focus:outline-none theme-focus-secondary" placeholder="admin@nexora.com" />
              </div>
            </label>
          )}
        ...
        <div className="mt-6 flex items-center justify-between gap-4 text-sm">
          {mode === 'register' && (
            <p className="text-gray-600">
              ...
              <Link to="/admin/login" className="theme-text-secondary font-bold">Login</Link>
            </p>
          )}
          {(mode === 'forgot' || mode === 'reset') && <Link to="/admin/login" className="theme-text-secondary font-semibold">Back to login</Link>}
          {mode === 'login' && (
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 w-full sm:justify-between">
              <p className="text-gray-600">
                Don't have an account?{' '}
                <Link to="/admin/register" className="theme-text-secondary font-bold">Register</Link>
              </p>
              <Link to="/admin/forgot-password" className="theme-text-secondary font-semibold">Forgot password?</Link>
            </div>
          )}
        </div>
      </div>
